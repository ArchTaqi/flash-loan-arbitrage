...
interface IFlashloan {
     ...
     * @param protocol A numeric identifier for the swap protocol, for arbitrage where we use many protocols (e-g., Uniswap, Sushiswap)-
     * @param data Arbitrary bytes data that may include additional parameters required by the protocol.
     * @param path An array of token addresses representing the swap path (tokenIn, intermediate tokens, tokenOut). e-g, ["WETH", "USDT", "UNI"]
     */
    struct Hop {
        uint8 protocol;
        bytes data;
